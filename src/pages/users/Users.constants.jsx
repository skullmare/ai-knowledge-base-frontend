...
const getPlatformRoleActions = ({ onEdit, onDelete }, row) => [
    {
        label: 'Редактировать',
        icon: EditIcon,
        permission: 'platformRoles.update',
        permissionMode: 'some',
        onClick: () => onEdit(row),
    },
    {
        ...
        icon: Delete,
        variant: 'delete',
        permission: 'platformRoles.delete',
        permissionMode: 'some',
        hidden: row.isSystem,
...
const getAgentRoleActions = ({ onEdit, onDelete }, row) => [
    {
        label: 'Редактировать',
        icon: EditIcon,
        permission: 'agentRoles.update',
        permissionMode: 'some',
        onClick: () => onEdit(row),
    },
    {
...
export const getPlatformRoleColumns = ({ onEdit, onDelete }) => [
    {
        key: 'name',
        label: 'Название',
        render: (value) => value ?? '—',
    ...
    },
    {
        ...
        render: (value) => {
            ...
            return count > 0 ? `${count} прав${count === 1 ? 'о' : count < 5 ? 'а' : ''}` : '—'
        },
    },
    {
        key: '_id',
        ...
        actions: true,
        render: (_, row) => (
            <DropdownActions actions={getPlatformRoleActions({ onEdit, onDelete }, row)} />
        ),
    },
]

export const getAgentRoleColumns = ({ onEdit, onDelete }) => [
    {
        key: 'name',
        label: 'Название',
        render: (value) => value ?? '—',
    ...
    },
    {
        ...
        actions: true,
        render: (_, row) => (
            <DropdownActions actions={getAgentRoleActions({ onEdit, onDelete }, row)} />
        ),
    },
]
